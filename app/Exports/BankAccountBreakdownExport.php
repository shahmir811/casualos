...
class BankAccountBreakdownExport
{
    ...
    private function build(): Spreadsheet
    {
        ...
        foreach ($this->orders as $i => $order) {
            ...
            $rowData[] = (float) $order->total_paid;
            $rowData[] = (float) $order->outstanding_balance;
        ...
        $totalRow[] = (float) $this->orders->sum('total_paid');
        $totalRow[] = (float) $this->orders->sum('outstanding_balance');
