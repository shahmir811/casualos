@extends('layouts.app')
@section('title', 'Add Stitching Unit')
@section('content')

<div class="flex items-center gap-3 mb-7">
    <a href="{{ route('stitching-units.index') }}" class="text-[#0066CC] hover:underline text-sm">Stitching Units</a>
    <span class="text-[#86868B]">/</span>
    <span class="text-[#1D1D1F] text-sm font-medium">Add Unit</span>
</div>

<div class="max-w-lg" x-data="{ paymentType: '{{ old('payment_type', 'per_piece') }}' }">
    <div class="card p-6">
        <h2 class="text-lg font-semibold text-[#1D1D1F] mb-6">New Stitching Unit</h2>
        ...
        <form method="POST" action="{{ route('stitching-units.store') }}" class="space-y-5">
            @csrf

            {{-- Unit Number (auto, read-only) --}}
            <div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-sm text-[#86868B]">Auto-assigned (next available number)</span>
                </div>
            </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <label class="relative cursor-pointer">
                        <input type="radio" name="payment_type" value="per_piece"
                               ...
                               {{ old('payment_type', 'per_piece') === 'per_piece' ? 'checked' : '' }}>
                    ...
                    <label class="relative cursor-pointer">
                        <input type="radio" name="payment_type" value="salary"
                               x-model="paymentType" class="sr-only peer"
                               {{ old('payment_type') === 'salary' ? 'checked' : '' }}>
                        <div class="flex flex-col items-center justify-center p-4 border-2 rounded-xl text-center transition-all
                                    border-[#E8E8ED] bg-white text-[#6E6E73]
