...
<div class="flex items-center gap-3 mb-7">
    ...
    <span class="text-[#86868B]">/</span>
    <span class="text-[#1D1D1F] text-sm font-medium">Edit Payment #{{ $order->order_number }}p{{ $payment->sequence_number }}</span>
</div>

<h1 class="text-2xl font-semibold tracking-tight text-[#1D1D1F] mb-1">Edit Payment #{{ $order->order_number }}p{{ $payment->sequence_number }}</h1>
...
<div class="card p-6"
     x-data="{
        paymentType: '{{ old('payment_type', $payment->payment_type) }}',
        amountDisplay: '{{ number_format((int) old('amount', $payment->amount), 0) }}',
        amountRaw: '{{ old('amount', $payment->amount) }}',
        advanceBalance: {{ (float) ($order->customer?->advance_credit_balance ?? 0) }},
        orderStatus: '{{ $order->status }}',
        totalAmount: {{ (float) $order->total_amount }},
        totalPaidExcludingThis: {{ (float) ($order->total_paid - $payment->amount) }},
        existingReceipts: {{ Illuminate\Support\Js::from(collect($payment->receipt_image ?? [])->map(fn($path) => [
            'path' => $path,
        ...
        get projectedOutstanding() { return Math.max(0, this.totalAmount - (this.totalPaidExcludingThis + Number(this.amountRaw || 0))); },
        get showDispatchWarning() { return this.orderStatus === 'dispatched' && this.projectedOutstanding > 0; },
    ...
    <form method="POST" action="{{ route('orders.payments.update', [$order, $payment]) }}" enctype="multipart/form-data" class="space-y-4">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <p x-show="isAdvance && Number(amountRaw || 0) > advanceBalance" x-cloak
                   class="mt-2 text-xs text-[#0071E3] bg-[#EFF8FF] border border-[#BEE3FF] rounded-lg px-3 py-2 leading-relaxed">
                    <template x-if="creditPortion > 0">
                        <span>PKR <span x-text="creditPortion.toLocaleString('en-US')" class="font-semibold"></span> will be applied from advance credit. </span>
                    </template>
                    <span>PKR <span x-text="paymentPortion.toLocaleString('en-US')" class="font-semibold"></span> will be recorded as an additional payment.</span>
                </p>
            </div>
            <div>
                ...
                <select name="payment_type" required class="apple-input" x-model="paymentType">
                    ...
                    <option value="advance">From Advance Credit @if(($order->customer?->advance_credit_balance ?? 0) > 0)(PKR {{ number_format($order->customer->advance_credit_balance, 0) }} available)@endif</option>
                </select>
            </div>
            ...
            <div x-show="needsBank" x-cloak class="sm:col-span-2">
                ...
                <select name="bank_account_id" class="apple-input" :required="needsBank">
                    ...
                    <option value="{{ $bank->id }}"
                        {{ old('bank_account_id', $payment->bank_account_id) == $bank->id ? 'selected' : '' }}>
                        {{ $bank->title }}
                    </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="date" name="payment_date" value="{{ old('payment_date', $payment->payment_date->format('Y-m-d')) }}" required class="apple-input">
            </div>
            <div>
                ...
                <input type="text" name="notes" value="{{ old('notes', $payment->notes) }}" class="apple-input" placeholder="e.g. first instalment">
            </div>
        </div>

        {{-- Dispatched-order warning — informational only, does not block submission --}}
        <p x-show="showDispatchWarning" x-cloak
           class="text-xs text-[#B45309] bg-[#FFFBEB] border border-[#FDE68A] rounded-lg px-3 py-2 leading-relaxed">
            This order has already been dispatched. Lowering this payment will reintroduce an outstanding balance of PKR
            <span x-text="projectedOutstanding.toLocaleString('en-US')" class="font-semibold"></span> — this is allowed, but you may want to follow up on collection.
        </p>
