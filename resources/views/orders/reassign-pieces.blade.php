...
<div class="flex items-center gap-3 mb-7">
    ...
    <span class="text-[#86868B]">/</span>
    <span class="text-[#1D1D1F] text-sm font-medium">Reassign Pieces</span>
</div>
...
<div class="max-w-2xl"
    ...
    <h1 class="text-2xl font-semibold tracking-tight text-[#1D1D1F] mb-1">Reassign Pieces</h1>
    <p class="text-[#6E6E73] text-sm mb-6">
        Source: Order #{{ $order->order_number }} · {{ $order->customer->name ?? '—' }} · {{ $order->catalogue->name ?? '—' }}
    </p>
    ...
    <div class="card p-8 text-center text-[#6E6E73] text-sm">
        No other active orders found for the <strong>{{ $order->catalogue->name }}</strong> catalogue to reassign pieces to.
    </div>
    @else

    <form method="POST" action="{{ route('orders.reassign.store', $order) }}" class="space-y-5">
        @csrf

        {{-- Target order --}}
        <div class="card p-6">
            <label class="block text-xs font-semibold text-[#6E6E73] uppercase tracking-widest mb-2">Assign To <span class="text-[#FF3B30]">*</span></label>
            <select name="target_order_id" class="apple-input" required>
                <option value="">— Select customer order —</option>
                @foreach($targetOrders as $target)
                <option value="{{ $target->id }}" {{ old('target_order_id') == $target->id ? 'selected' : '' }}>
                    {{ $target->customer->name ?? '—' }} — Order #{{ $target->order_number }}
                    ({{ ucfirst(str_replace('_', ' ', $target->status)) }})
                </option>
                @endforeach
            </select>
            <p class="text-xs text-[#86868B] mt-2">Only active orders for the same catalogue are shown.</p>
        </div>
        ...
        <div class="card overflow-hidden">
            <div class="px-5 py-4 border-b border-[#F2F2F7] flex items-center justify-between">
                <div>
                    <h3 class="text-sm font-semibold text-[#1D1D1F]">Pieces to Reassign</h3>
                    <p class="text-xs text-[#6E6E73] mt-0.5">Select design, size, and quantity to move</p>
                </div>
            ...
            <div x-show="items.length === 0" class="px-5 py-8 text-center text-[#86868B] text-sm">
                Click <strong>+ Add Item</strong> to specify pieces to reassign.
            </div>
        </div>

        {{-- Live total --}}
        <div x-show="items.length > 0" x-cloak class="card p-5">
            <div class="flex justify-between text-sm">
                <span class="text-[#6E6E73]">Amount added to target order</span>
                <span class="font-semibold text-[#0071E3]" x-text="formatPkr(totalAdded)"></span>
            </div>
        </div>
        ...
        <div>
            ...
            <textarea name="notes" rows="2" class="apple-input" placeholder="Reason for reassignment...">{{ old('notes') }}</textarea>
        </div>

        <div class="flex gap-3">
            <button type="submit" class="btn-primary" x-bind:disabled="items.length === 0">Confirm Reassignment</button>
