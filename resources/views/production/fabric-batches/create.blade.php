...
<div class="max-w-2xl"
    ...
    <h1 class="text-2xl font-semibold tracking-tight text-[#1D1D1F] mb-6">Log Fabric Batch Arrival</h1>
    ...
    <form method="POST" action="{{ route('fabric-batches.store') }}" class="space-y-5">
        @csrf
        ...
        <div class="card p-6 space-y-5">
            ...
            <div>
                <label class="block text-xs font-semibold text-[#6E6E73] uppercase tracking-widest mb-2">Arrival Date</label>
                <input type="date" name="arrival_date" value="{{ old('arrival_date', date('Y-m-d')) }}" class="apple-input" required>
            </div>
            ...
            <div>
                ...
                <textarea name="notes" rows="2" class="apple-input" placeholder="e.g. Fabric quality notes...">{{ old('notes') }}</textarea>
            </div>
        </div>

        {{-- Design Items — shown after catalogue selected --}}
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('fabric-batches.index') }}" class="btn-secondary">Cancel</a>
        </div>
    </form>
</div>
