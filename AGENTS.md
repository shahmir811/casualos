...
**`FreePieceController`** (routes `free-pieces.index`, `free-pieces.assign`, `free-pieces.store` — admin + accountant, Sales sidebar section):
- **`index()`** — shows the active catalogue's free-piece grid (design × size). The per-size "available" total is computed as the **minimum quantity across every design carrying that size**, not a sum — free pieces are expected to always land uniformly across every design in the order that generated them, so the min is a defensive cap against drift rather than a real aggregation.
- **`assign()`** — one uniform per-size quantity (`qty_xs`..`qty_xl`, capped client-side by the grid's "available" numbers) applied identically to every design with free stock, mirroring how a real order's sizing works. One combined type-to-filter combobox lists both valid targets — existing non-`dispatched`/non-`cancelled` orders on the catalogue (labeled `"<customer> — Order #<n> (<status>)"`) and customers with no order yet on this catalogue (labeled `"<customer> — New Order"`) — so the admin doesn't need to know in advance which bucket a name belongs to.
- **`store()`** — locks every `FreePiece` row for the catalogue (`lockForUpdate()`), re-validates the requested per-size quantities don't exceed any design's available stock, decrements/deletes the rows, then either:
