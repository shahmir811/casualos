...
**Country pricing (admin-only):** `design_country_prices` table — one price per design per destination country (`Customer::COUNTRIES`: Australia, Bangladesh, Canada, Kuwait, Malaysia, Norway, Oman, Pakistan, Qatar, Saudi Arabia, UAE, UK, USA — expanded from the original 7 by commit `ff4b9ec`, which added Bangladesh, Kuwait, Oman, Qatar, then further expanded 2026-09-11 with Malaysia and Norway, see rule 5.36; corrected here 2026-09-11, this section previously still listed the pre-Malaysia/Norway 11), unique on `(design_id, country)`. Managed from the **Country Pricing** screen (`country-pricing.index` / `country-pricing.store`, admin only), scoped to the active catalogue (same `session('active_catalogue_id')` pattern as other production screens). Leaving a cell blank deletes that design+country's price row. `Customer::CURRENCY_SYMBOLS` maps each country to its tag currency label: Pakistan → `Rs.`, UK → `£`, USA → `US $`, Australia → `AUS $`, Canada → `CAD $`, Saudi Arabia → `SR`, UAE → `AED`, Bangladesh → `TK`, Kuwait → `KWD`, Malaysia → `RM`, Norway → `NOK`, Oman → `OMR`, Qatar → `QAR`.
...
### 5.35 Catalogue Book — Per-Catalogue Lookbook PDF

**Purpose:** the Casualite owner sends customers a full lookbook (a bound PDF of the season's designs, seen up to ~200MB) alongside the WhatsApp order link. Previously this was shared manually, outside CasualiteOS. This adds a place to upload it once per catalogue and a place for customers to view it — but deliberately **not** as a public no-auth link like the HD Gallery (rule 5.27): the owner confirmed the book should only be viewable by an already-identified customer, inside the portal or the app, not shared as a bare link.

**One file per catalogue, not a gallery.** `catalogues.catalogue_book_path` (nullable S3 path) plus `catalogue_book_original_filename`, `catalogue_book_file_size` (`unsignedBigInteger`, same column type HD Gallery uses for its own large files), `catalogue_book_uploaded_by` (nullable FK to `users`, `nullOnDelete`), `catalogue_book_uploaded_at` — migration `2026_09_09_000004`. Re-uploading replaces the existing file (old S3 object deleted once the new one is confirmed saved); there is no version history, matching the "one file, replaced in place" precedent of `catalogues.cover_photo`, not the many-rows precedent of `catalogue_hd_images`.

**Upload — direct-to-S3 presigned PUT, same mechanic as the HD Gallery (rule 5.27), for the same reason: size.** A 60–200MB (up to 300MB accepted) PDF cannot go through this app's shared-hosting PHP process (`upload_max_filesize`/`post_max_size`/execution time all cap out well below that). `CatalogueBookController::presign()` issues a presigned PUT (`Storage::disk('s3')->temporaryUploadUrl()`, 60-minute expiry — longer than HD Gallery's 30 minutes, since a single PDF this large can take longer to transfer than an individual HD photo) for key `catalogue-books/{catalogue_id}/{uuid}.pdf`. `store()` re-checks existence and reads the authoritative size back from S3 (`Storage::disk('s3')->size()`) rather than trusting the client, rejecting (and deleting the object) if it exceeds `CatalogueBookController::MAX_BYTES` (300MB — headroom above the ~200MB largest book confirmed with the client, mirroring HD Gallery's 1GB constant being headroom above its own stated range). **No new CORS command was needed** — `s3:configure-gallery-cors` (rule 5.27) already applies its policy bucket-wide with no key-prefix restriction (S3 CORS rules can't be scoped by prefix), so an environment that has already run it for HD Gallery uploads is already correctly configured for Catalogue Book uploads too.

**Upload UI lives on `catalogues/show.blade.php`, not a separate management screen.** Unlike HD Gallery (which needed its own index page to manage many images), a catalog book is a single file, so it's a card on the catalogue's own show page — same placement tier as the "Shareable Order Link" card, gated to `admin|production_manager|creative_head` (the same tier as catalogue create/edit access, per `CatalogueController::adminOrProductionManager()` — broader than HD Gallery's `admin|creative_head`, since the ask here was explicitly "any user who has the right to create a catalog"). The card shows an upload button when no book exists, or the filename/size/uploader/date plus View/Replace/Delete when one does. `catalogBookUploader()` (Alpine, in the same file) is a single-file simplification of HD Gallery's `hdGalleryUploader()` — no thumbnail step, no queue, just presign → XHR `PUT` with an `upload.onprogress` progress bar → `store()`.

**Viewing is always via a fresh short-lived presigned S3 URL with `inline` disposition — never `attachment`, and never a stored/reusable link.** Inline disposition means the browser renders the PDF using its native viewer instead of downloading it, satisfying "embedded in-browser PDF viewer" without needing a custom iframe wrapper page (which would add fragility around framing/CORS for no real benefit). Three access points, all generating the redirect/URL on demand rather than embedding one at list-load time:
- **Staff (admin panel):** `catalogues.book.view` (`CatalogueBookController::view()`), same role tier as upload, 10-minute presign, `redirect()->away()` — same mechanic as `GalleryController::download()` but `inline` instead of `attachment`.
- **Customer portal (web):** `portal.catalogue-book` (`CustomerPortalController::catalogueBook()`), gated by the same device-cookie check `pushSubscribe()` uses (`resolveDevice()` — only an already-verified device can reach it) plus an explicit `$order->customer_id === $customer->id` ownership check (404, not 403, on a mismatch — consistent with the "don't confirm another customer's data exists" convention used elsewhere, e.g. `Api\OrderController::show()`). Surfaced as a "Catalog Book" link on each order card in `portal/dashboard.blade.php`, directly below the existing "HD Photos" link, shown only when `$order->catalogue->catalogue_book_path` is set.
- **Mobile app:** `api.catalogues.book` (`Api\CatalogueController::book()`, `auth:sanctum`), returns `{"url": "..."}` JSON rather than a redirect (the app can't follow a server-side redirect the way a browser tab can) with a 10-minute presign. `has_catalogue_book` (a plain boolean, no S3 call) was added to both `CatalogueResource` and `CatalogueSummaryResource` so the app can decide whether to show a "View Catalog Book" action without hitting `book()` for every catalogue in a list — the app only calls `book()` right before opening the viewer, avoiding the failure mode of an embedded presigned URL expiring while the customer is still browsing the catalogue list.

**Deliberately not built:** a public no-auth share link (unlike HD Gallery — this was the one point explicitly ruled out for this feature), multi-version history, and a "browse before ordering" surfacing point on the **web** portal (the web portal has no catalogue-browsing screen at all — only the mobile app does, per rule 5.34's Slice 3 catalogue endpoints — so the web side is necessarily tied to an existing order, same as HD Photos).

---

### 5.36 Destination Countries — API-Driven Single Source

**Purpose:** `Customer::COUNTRIES` (Section 4) was already the one array every screen and validation rule in *this* repo reads from — the web create/edit forms, `CustomerController`'s `in:` validation, `Api\AuthController::signup()`'s validation, and the Country Pricing screen all pointed at it correctly. The actual duplication was in the separate `casualite-app` React Native repo: `app/(auth)/signup.tsx` kept its own hardcoded `COUNTRIES` array for the self-signup country picker (rule 5.34), manually kept in sync by whoever edited it, with a comment admitting as much ("must match the web portal's dropdown exactly"). Adding a country previously meant a coordinated edit across two repos plus an app store release for the mobile side. Prompted by the client asking to add Malaysia and Norway (2026-09-11) and wanting future countries to be easier to add.

**Fix:** `GET /api/countries` (`api.countries.index`, `Api\CountryController::index()`, public, no auth — the signup screen renders before a customer has any credential) returns `{"countries": [...]}`, read straight off `Customer::COUNTRIES`. No new table, no admin CRUD screen — this was a deliberate scope call (see below): the constant stays the single source of truth in code, this endpoint just lets a second codebase read it at runtime instead of hardcoding a copy. Adding a country is still a one-line edit to `Customer::COUNTRIES` (and, if it needs a piece-tag currency symbol, to `Customer::CURRENCY_SYMBOLS` alongside it) followed by a deploy — but from now on that's the *only* place it needs to happen; the mobile app picks it up on its next request with no release required.

**`casualite-app`'s `signup.tsx` now fetches on mount** instead of hardcoding: `apiClient.get<CountriesResponse>('/api/countries')` (new `CountriesResponse` type in `lib/types.ts`) populates the picker's `FlatList`, with a loading state on the Country field itself (disabled, "Loading countries…") and a retry affordance inside the picker modal if the fetch fails — the signup form's other fields remain usable while this loads or if it errors; only country selection is blocked.

**Scope explicitly not taken:** a DB-backed, admin-editable country list (new `countries` table, CRUD screen, no-deploy country additions) was considered and rejected for this round in favor of the code-constant + API approach — noted here in case a future request revisits wanting country management without a deploy.
...
### 5.39 Customer Mobile App Usage Tracking (iOS/Android, Admin-Only)

**Purpose:** the owner asked for a way to tell whether a given customer is actually using the `casualite-app` mobile app, and if so, on iOS or Android — visible nowhere before this. `expo_push_tokens` (Section 9's Push Notifications slice) was deliberately **not** reused as the source of truth: that table only gains a row when a customer grants push permission, which undercounts real app usage — a customer who opened the app, browsed catalogues, and placed an order, but denied notifications, would show as "not using the app." This feature tracks usage independent of push opt-in.

**Schema — two nullable columns on `customers`, not a separate history table:** migration `2026_09_22_000003_add_app_usage_to_customers_table` adds `app_platform` (enum `ios`/`android`) and `app_last_seen_at` (timestamp). Deliberately a "most recent platform + last-seen" snapshot, not a per-session log — the ask was "is this customer using the app, and on what platform," not a usage history, so a two-column addition to the existing model was preferred over introducing a new table.

**Captured at login:** `Api\AuthController::verify()`'s customer branch now accepts an optional `platform` field (`nullable|string|in:ios,android`) in the request body. On a successful match, after the bearer token is issued, if `platform` was sent, `$customer->update(['app_platform' => ..., 'app_last_seen_at' => now()])` runs.

**Refreshed on every authenticated request after that — this is the part that matters, not just login.** A customer's Sanctum bearer token (issued by `verify()`) is long-lived and the app only re-validates it against `GET /api/me` on cold start (per Section 9's "Mobile API — Authentication" slice) rather than calling `verify()` again — so anchoring `app_last_seen_at` to `verify()` alone would freeze at first install and never update again. `App\Http\Middleware\TrackCustomerAppUsage` (aliased `track-app-usage` in `bootstrap/app.php`) sits on the entire `auth:sanctum` group in `routes/api.php` — every customer-facing API call (`/api/me`, `/api/orders`, `/api/ledger`, `/api/announcements`, `/api/catalogues`, `/api/push-tokens`, `/api/size-chart`, etc.) passes through it. It reads an `X-App-Platform` header (`ios`/`android`) the app is expected to send as a default header on every request:
- `app_last_seen_at` is set to `now()` unconditionally on every request that reaches it, header or not.
- `app_platform` is only updated when the header is present and valid — a missing header never clears a previously known platform, it just leaves it as the last value seen.

The middleware no-ops for any non-`Customer` authenticated user, though today this route group is customer-only (staff never receive a Sanctum token — see rule 5.33).

**Admin-only visibility — narrower than the existing `$hideFinancials` gate.** `CustomerController::index()`/`show()` already compute `$hideFinancials` for `production_manager`; this feature is not tied to that flag. `CustomerController::show()` separately computes `$isAdmin = Auth::user()->role === 'admin'` and passes it to the view. `customers/show.blade.php` renders the usage line only `@if($isAdmin)` — `accountant` and `production_manager` (the only other two roles with `customers.show` access) never see it, even though `accountant` isn't subject to `$hideFinancials` elsewhere on this same page. This was an explicit, narrower scoping decision from the owner, not a byproduct of the existing financials gate — don't fold this into `$hideFinancials` or widen it to `accountant` without asking first.

**Display:** a blue pill badge "Using App · iOS" / "Using App · Android" plus "Last seen {{ diffForHumans }}" when `app_platform` is set; plain gray uppercase caption text "Not using the app" (deliberately **not** rendered with the `.badge` CSS class) when it's null. The `.badge` class carries `padding: 0.2rem 0.6rem` for a pill look, which reads as unwanted left padding on a caption with no visible pill background — this was caught and fixed after the first pass rendered "Not using the app" as a `.badge` span.

**No mobile-app-repo changes are included here — this is a backend-only, cross-repo contract.** `casualite-app` must (1) send `platform: Platform.OS === 'ios' ? 'ios' : 'android'` in the `POST /api/auth/verify` request body at login, and (2) set `X-App-Platform` as a default header on its HTTP client so every subsequent authenticated request carries it automatically. Until the app side ships both, every customer will show "Not using the app" regardless of actual usage — this is expected, not a backend bug, until that companion change lands.

**Tests:** `tests/Feature/Api/AuthTest.php` covers: `verify()` records platform + last-seen when sent, `verify()` without a platform leaves both columns untouched, a later authenticated request's `X-App-Platform` header overrides a previously recorded platform and refreshes last-seen, and a request with no header still refreshes last-seen without clearing a known platform. The other `Api\*` test suites (`OrderTest`, `LedgerTest`, `CatalogueTest`, `OrderPlacementTest`, `PushTokenTest`, `AnnouncementTest`, `SizeChartTest`) all route through the `auth:sanctum` group and therefore through this middleware too — each of their hand-built `customers` schemas (see those suites' own docblocks on why they don't run full migrations) needed the two new columns added so the middleware's `update()` call doesn't fail against a missing column.
...
| `catalogues.book.presign`        | `POST /catalogues/{catalogue}/book/presign` | Presigned S3 PUT URL for the catalog book PDF (admin + production_manager + creative_head, see rule 5.35) |
| `catalogues.book.store`          | `POST /catalogues/{catalogue}/book`         | Register a catalog book already uploaded to S3 (admin + production_manager + creative_head) |
| `catalogues.book.view`           | `GET /catalogues/{catalogue}/book/view`     | Staff view — redirects to a short-lived presigned S3 URL, inline disposition (admin + production_manager + creative_head) |
| `catalogues.book.destroy`        | `DELETE /catalogues/{catalogue}/book`       | Delete the catalog book (S3 + DB) (admin + production_manager + creative_head) |
...
- **Timeline / Announcements — CasualOS admin compose screen** (2026-08-18): the client's own name for the customer-facing feed is "Timeline" — a Facebook-style scrolling feed of admin posts (new-catalogue announcements, customer-highlight photos, etc.), delivered as a push notification plus an in-app history row. The mobile API + notification plumbing above shipped first; this is the CasualOS side that was missing — previously the only way to send one was `php artisan announcements:send` from a terminal.
  ...
  - **Voice notes on announcements** (2026-09-12): an announcement can now optionally carry a WhatsApp-style voice note alongside (or instead of) its image(s) — all four image/audio combinations are valid, admin history is kept permanently (posts are never edited/deleted, same as the rest of Timeline), scoped deliberately to the admin compose screen + admin history + the mobile app's feed. **The customer portal has no Timeline/announcements UI at all** (confirmed while scoping this — `AnnouncementNotification` was mobile-app-only from the start, see its own docblock), so no portal changes were made; a portal-web Timeline is a separate, larger, not-yet-requested feature.
    - **Schema**: `announcements` gains `audio_path`, `audio_original_filename`, `audio_file_size` (migration `2026_09_12_193232_add_audio_to_announcements_table`), mirroring the existing `image_paths` handling. `AnnouncementService::send()` gained three trailing optional params for these; `SendAnnouncement`'s CLI path grew a matching `--audio=` option so both callers stay in sync, per this feature's own "shared service, two callers" convention.
    - **Upload — presigned direct-to-S3, unlike the synchronous image upload.** Voice notes are deliberately uncapped in size ("no cap, just like a WhatsApp voice note" was the explicit requirement) and can't safely go through this app's shared-hosting PHP process for the same reason Catalog Book PDFs (rule 5.35) don't. `AnnouncementController::presignAudio()` (route `announcements.audio.presign`) issues a presigned PUT for key `announcements/{uuid}.{ext}`, validating only that the client-reported `content_type` matches `^audio\/` — no format allow-list beyond that. The browser PUTs directly to S3 *before* the compose form is submitted; the main `store()` POST carries the resulting key as a hidden `audio_key` field (plus `audio_original_filename`), and `resolveUploadedAudio()` re-verifies the object actually exists on S3 (and reads its real size) before ever creating the `Announcement` row — same "trust S3, not the client" convention as `CatalogueBookController::store()`. Images are unchanged (still a normal synchronous multipart upload in the same request) since only audio has the uncapped-size requirement.
    - **Push indicator ("has_audio")**: `AnnouncementNotification::toExpoPush()` prefixes the push body with `"🎤 Voice message · "` whenever `audioPath` is set (a push notification is text-only — there's nothing to actually play from the OS notification), and also sets a structured `has_audio` boolean in the `data` payload so the app can render its own mic icon without parsing the body string. `toDatabase()` carries `audio_path` through unchanged so the in-app feed always has the real path regardless of what the push text said.
    - **Playback — a client-side-decoded waveform player, no server-side audio pipeline.** `resources/views/admin/announcements/index.blade.php`'s `@push('scripts')` defines two Alpine factories sharing a `computeWaveformPeaks()`/`formatAudioTime()` pair: `announcementComposer()` (the compose form — decodes the local `File` object directly via `AudioContext.decodeAudioData()` right after upload, no re-fetch) and `waveformPlayer(url)` (used for any already-sent voice note — fetches the public S3 URL, same `Storage::url()` convention as images, no presigning needed since announcement media has always been public). The latter is factored into `resources/views/admin/announcements/_waveform-player.blade.php` and `@include`d once per announcement row in both the admin history and — this is the firm requirement this feature was built around — that history is permanent and always playable, never just an icon, matching Timeline's "never edited or deleted" posts.
    - **Mobile app**: `AnnouncementResource` gained `has_audio` (bool) and `audio_url` (nullable, `Storage::url()`), so `GET /api/announcements` carries everything the RN app's own player needs; that player's UI is out of scope for this repo (see `casualite-app`).
    - **Tests**: `tests/Feature/AnnouncementServiceTest.php` covers `send()` storing/omitting audio fields; `tests/Feature/Api/AnnouncementTest.php` covers `has_audio`/`audio_url` in the API response and the Expo push body-prefix/flag behavior. No new web-controller HTTP test was added, consistent with this feature's existing "no Feature-test coverage of the admin route" call above.
...
- **Customer Mobile App Usage Tracking** (2026-09-22): admin-only visibility (not `accountant`, not `production_manager`) on the Customer show page into whether a customer is using `casualite-app`, and on iOS or Android. Two nullable columns on `customers` — `app_platform`, `app_last_seen_at` — captured once at login (`Api\AuthController::verify()`'s new optional `platform` field) and refreshed on every subsequent authenticated API call via a new `track-app-usage` middleware reading an `X-App-Platform` header, rather than relying on `expo_push_tokens` (which only exists for customers who granted push permission, undercounting real usage). Deliberately independent of the existing `$hideFinancials` role gate. The mobile-app side of this (sending `platform` at login, setting `X-App-Platform` as a default request header) is a separate `casualite-app` change not included in this repo — until it ships, every customer shows "Not using the app." See rule 5.39 for full detail.
...
- `2026_09_22_000003_add_app_usage_to_customers_table` — adds nullable `app_platform` (enum `ios`/`android`) and `app_last_seen_at` (timestamp) to `customers` — tracks general mobile-app usage independent of push-notification opt-in; see rule 5.39
